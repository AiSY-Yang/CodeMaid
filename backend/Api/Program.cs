			...
			await Task.Run(async () =>
					{
						...
						try
						{
							var scope = app.Services.CreateScope();
							var context = scope.ServiceProvider.GetRequiredService<MaidContext>();
							var project = context.Projects.First(x => x.Id == 4);
							...
							void GetInfo(string path)
							{
								...
								Console.WriteLine($"{path}文件{files.Length}个");
								foreach (var file in files)
								{
									...
									var projectDirectoryFile = new ProjectDirectoryFile()
									{
									...
									context.Add(projectDirectoryFile);
								}
								context.SaveChanges();
							}
